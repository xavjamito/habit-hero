...
export default function CalendarPage() {
  ...
  const completionsByDate = completions.reduce<Record<string, Completion[]>>((acc, completion) => {
                ...
                <Card>
                  ...
                  <CardContent>
                    <Calendar
                      ...
                      components={{
                        DayContent: ({ day }) => {
                          const { percentage, completedCount, totalCount } = getDayCompletionInfo(day);
                          
                          // Skip showing info for days without habits or outside current month
                          if (totalCount === 0 || day.getMonth() !== date.getMonth()) {
                          ...
                          return (
                            <div className="flex flex-col items-center justify-center h-full w-full">
                              ...
                              {completedCount > 0 && (
                                <div className={`text-xs mt-1 px-1.5 py-0.5 rounded-full ${bgColorClass}`}>
                ...
                <Card>
                  ...
                  <CardContent>
                    {habits.length === 0 ? (
                      ...
                      <div className="text-center py-6">
                        <p className="text-muted-foreground">No completed habits on this day.</p>
                      </div>
                    ) : (
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="bg-gray-50 p-3 rounded-md">
                          ...
                          <p className="text-2xl font-semibold">
                            {habits.length > 0 
                              ? `${Math.round((selectedDayCompletions.length / habits.length) * 100)}%` 
                              : '0%'}
                          </p>
                        </div>
                        <div className="bg-gray-50 p-3 rounded-md">
                          <p className="text-sm text-muted-foreground">Completed</p>
                          <p className="text-2xl font-semibold">{selectedDayCompletions.length}/{habits.length}</p>
                        </div>
                      </div>
                    </div>
                  </CardContent>
