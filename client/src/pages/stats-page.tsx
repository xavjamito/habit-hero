...
import { Loader2, BarChart2, TrendingUp, Calendar, Activity } from "lucide-react";
import { calculateCompletionPercentage, calculateStreak } from "@/lib/utils";
import { motion } from "framer-motion";
import { format, startOfWeek, endOfWeek, eachDayOfInterval, subMonths, startOfMonth, addDays, isSameDay } from "date-fns";
...
export default function StatsPage() {
  ...
  // Fetch completions
  const { data: completions = [], isLoading: isLoadingCompletions } = useQuery<Completion[]>({
  ...
  const longestStreak = habits.length > 0
    ? Math.max(...getStreakData().map(d => d.streak))
    : 0;
  ...
  const mostConsistentHabit = habitCompletionData.length > 0
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen overflow-hidden">
              ...
              >
                <Card>
                  ...
                  <CardContent>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {totalCompletions} of {totalPossibleCompletions} habits completed
                    </p>
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {habits.length > 0 ? "Currently tracking" : "No habits added yet"}
                    </p>
                  </CardContent>
                </Card>
              </motion.div>
              ...
              <Tabs defaultValue="overview" className="space-y-4">
                ...
                <TabsContent value="overview" className="space-y-4">
                  ...
                  >
                    <Card>
                      <CardHeader>
                        <CardTitle>Habit Completion {timeRange === "week" ? "This Week" : "This Month"}</CardTitle>
                      </CardHeader>
                      ...
                      <CardContent>
                        <div className="h-80 flex items-center justify-center">
                          {habitCompletionData.length === 0 ? (
                            <p className="text-muted-foreground">No habit data to display</p>
                          ) : (
                            <ResponsiveContainer width="100%" height="100%">
                              <PieChart>
                                <Pie
      ...
      <BottomNavbar currentPath="/stats" />
    </div>
  );
}
